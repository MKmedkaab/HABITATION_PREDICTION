...
def main():
    ...
    Date = st.text_input("Date (2021)", "Type Here")
    Property_Type = st.sidebar.selectbox("Property type: D = Detached, S = Semi-Detached, T = Terraced, F = Flats/Maisonettes, O = Other", ("D", "S", "T", "F", "O"))
    Old_New = st.sidebar.selectbox("Old/New: Y = a newly built property, N = an established residential building", ("Y", "N"))
    Duration = st.sidebar.selectbox("Duration: F = Freehold, L= Leasehold", ("F", "L"))
    PAON=st.text_input("Primary Addressable Object Name", "Type Here")
    County= st.text_input("County", "Type Here")
    PPD_Category_Type=st.sidebar.selectbox("PPD Category Type: A = Standard Price Paid entry, B = Additional Price Paid entry", ("A", "B"))
